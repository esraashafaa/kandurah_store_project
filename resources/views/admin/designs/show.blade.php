    ...
    <ol class="inline-flex items-center space-x-1 md:space-x-3 rtl:space-x-reverse">
        ...
        <li>
            <div class="flex items-center">
                ...
                <a href="{{ route('dashboard.designs.index') }}" class="text-gray-600 hover:text-indigo-600">التصاميم</a>
            </div>
        </li>
        <li aria-current="page">
            <div class="flex items-center">
                <i class="fas fa-chevron-left text-gray-400 mx-2"></i>
                <span class="text-gray-500">{{ $design->name ?? 'تصميم #' . $design->id }}</span>
            </div>
        </li>
    </ol>
</nav>
...
<div class="bg-gradient-to-r from-purple-600 to-pink-600 rounded-xl shadow-lg p-6 mb-6 text-white">
    <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
            <h1 class="text-3xl font-bold mb-2">{{ $design->name ?? 'تصميم #' . $design->id }}</h1>
            <p class="text-purple-100">تم الإنشاء في {{ $design->created_at->format('Y-m-d H:i') }}</p>
        </div>
...
<div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
    ...
    <div class="lg:col-span-2 space-y-6">
        ...
        <div class="bg-white rounded-xl shadow-sm p-6">
            <h2 class="text-xl font-bold text-gray-800 mb-4">صور التصميم</h2>
            ...
            <div class="grid grid-cols-2 gap-4">
                ...
                <div class="relative group aspect-square rounded-lg overflow-hidden bg-gray-100">
                    ...
                    <div class="absolute inset-0 bg-black bg-opacity-0 group-hover:bg-opacity-50 transition-all duration-300 flex items-center justify-center opacity-0 group-hover:opacity-100">
                        <a href="{{ Storage::url($image->image_path) }}" target="_blank" class="bg-white text-gray-800 p-3 rounded-full hover:bg-gray-100 transition">
            ...
            <div class="text-center py-12">
                ...
                <p class="text-gray-500">لا توجد صور لهذا التصميم</p>
            </div>
            @endif
        </div>
        ...
        <div class="bg-white rounded-xl shadow-sm p-6">
            ...
            <div class="grid grid-cols-2 md:grid-cols-3 gap-4">
                ...
                <div class="border border-gray-200 rounded-lg p-4 hover:border-indigo-500 transition">
                    ...
                    <img src="{{ Storage::url($option->image_path) }}" alt="{{ $option->name }}" class="w-full h-24 object-cover rounded-lg mb-2">
                    @endif
                    <p class="text-sm font-semibold text-gray-900">{{ $option->name }}</p>
                    <p class="text-xs text-gray-500">{{ $option->type ?? 'خيار' }}</p>
                </div>
                @endforeach
            </div>
        </div>
        ...
        <div class="bg-white rounded-xl shadow-sm p-6">
            <h2 class="text-xl font-bold text-gray-800 mb-4">المقاسات</h2>
    ...
    <div class="space-y-6">
        ...
        <!-- Design Details -->
        <div class="bg-white rounded-xl shadow-sm p-6">
            <h2 class="text-xl font-bold text-gray-800 mb-4">تفاصيل التصميم</h2>
            <div class="space-y-3">
                <div>
                    <p class="text-sm text-gray-600">رقم التصميم</p>
                    <p class="font-semibold text-gray-900">#{{ $design->id }}</p>
                </div>
                <hr>
                <div>
                    <p class="text-sm text-gray-600">نوع القماش</p>
                    <p class="font-semibold text-gray-900">{{ $design->fabric_type ?? 'غير محدد' }}</p>
                </div>
                <hr>
                <div>
                    <p class="text-sm text-gray-600">اللون</p>
        ...
        <div class="bg-white rounded-xl shadow-sm p-6">
            ...
            <div class="flex items-center gap-3 mb-4 pb-4 border-b border-gray-200">
                @if($design->user->profile_image)
                    <img src="{{ Storage::url($design->user->profile_image) }}" alt="{{ $design->user->name }}" class="w-16 h-16 rounded-full object-cover">
                @else
            ...
            <div class="space-y-2">
                <div class="flex items-center gap-2 text-sm">
                    ...
                    <span class="text-gray-700">{{ $design->user->phone ?? 'غير محدد' }}</span>
                </div>
                <div class="flex items-center gap-2 text-sm">
                    <i class="fas fa-palette text-gray-400 w-5"></i>
                    <span class="text-gray-700">{{ $design->user->designs()->count() }} تصميم</span>
                </div>
            </div>
        ...
        <div class="bg-white rounded-xl shadow-sm p-6">
            ...
            <div class="space-y-2">
                <button onclick="sendNotification()" class="w-full text-right px-4 py-3 hover:bg-gray-50 rounded-lg transition flex items-center gap-3">
                    ...
                    <span class="text-gray-700">إرسال إشعار للعميل</span>
                </button>
                <button onclick="printDesign()" class="w-full text-right px-4 py-3 hover:bg-gray-50 rounded-lg transition flex items-center gap-3">
                    ...
                    <span class="text-gray-700">طباعة التصميم</span>
                </button>
                <button onclick="deleteDesign({{ $design->id }})" class="w-full text-right px-4 py-3 hover:bg-gray-50 rounded-lg transition flex items-center gap-3">
                    ...
                    <span class="text-gray-700">حذف التصميم</span>
                </button>
            </div>
        </div>
...
function sendNotification() {
    alert('سيتم فتح نافذة إرسال إشعار');
}
...
function deleteDesign(designId) {
    ...
    .then(data => {
        if (data.success) {
            ...
            alert(data.message || 'حدث خطأ أثناء حذف التصميم');
        }
    })
    .catch(error => {
