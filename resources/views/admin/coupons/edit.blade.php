...
<div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
    <div>
        <h1 class="text-3xl font-bold text-gray-900">تعديل كوبون</h1>
        <p class="text-gray-600 mt-1">تعديل بيانات الكوبون: {{ $coupon->code }}</p>
    </div>
    ...
    <form action="{{ route('admin.coupons.update', $coupon) }}" method="POST">
        ...
        <div class="mt-6">
            ...
            <textarea id="description" 
                      ...
                      placeholder="وصف الكوبون (اختياري)">{{ old('description', $coupon->description) }}</textarea>
            @error('description')
                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
            @enderror
        </div>
