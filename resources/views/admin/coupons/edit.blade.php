...
@section('title', __('coupons.edit_title'))
...
<div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
    <div>
        <h1 class="text-3xl font-bold text-gray-900">{{ __('coupons.edit_title') }}</h1>
        <p class="text-gray-600 mt-1">{{ __('coupons.edit_title') }}: {{ $coupon->code }}</p>
    </div>
    ...
    <form action="{{ route('admin.coupons.update', $coupon) }}" method="POST">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
                ...
                @error('max_usage')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
                <p class="mt-1 text-sm text-gray-500">الاستخدام الحالي: {{ $coupon->usage_count }}</p>
            </div>
        ...
        <div class="mt-6">
            ...
            <textarea id="description" 
                      ...
                      placeholder="{{ __('coupons.fields.description_placeholder') }}">{{ old('description', $coupon->description) }}</textarea>
            @error('description')
                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
            @enderror
        </div>
